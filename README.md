...
- The multiprocess proof validates the failover, stale-authority rejection, no-reclaim, and coordinator restart reconciliation over real processes. Not separately orchestrated in the reference runner: a live old-worker-fencing beat with the old worker's process left alive while its control socket is severed, an in-flight "accepted but response held then worker dies" barrier within the multiprocess runner, a stateful checkpoint-restore subprocess proof, and a dedicated failback-exercising transition (the failback/anti-flapping policy is modeled and served by the same transaction machinery, and stateful-restore eligibility is enforced by the selection rules, but those two are not executed as separate multiprocess scenarios).
