...
- Still not executed as separate multiprocess scenarios: a stateful checkpoint-restore subprocess proof and a dedicated failback-exercising transition (failback/anti-flapping is modeled and served by the same transaction machinery, and stateful-restore eligibility is enforced by the selection rules). The routed-verification-through-a-fresh-gateway leg of the gateway-restart scenario is reported separately (the core gateway-restart claims pass; a residual route re-acquisition issue prevents that one leg from serving in-session).
